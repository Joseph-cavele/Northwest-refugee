...
function Owner({ row }: { row: DocumentRow }) {
  ...
  return (
    <div className="min-w-0">
      ...
      <span className="block truncate font-mono text-sm text-subtle">{owner.referenceCode}</span>
    </div>
  );
}
          ...
          <ul className="divide-y divide-line md:hidden">
            {rows.map((row) => (
              <li key={row._id} className="flex flex-col gap-2 px-4 py-3.5">
                <Owner row={row} />
                <p className="text-base text-body">{DOCUMENT_KIND_LABELS[row.kind]}</p>
                ...
                {rows.map((row) => {
                  ...
                  return (
                    <tr key={row._id} className="border-b border-line last:border-0 hover:bg-ink-25">
                      ...
                      <td className="max-w-[16rem] truncate px-4 py-3 text-sm text-subtle">
                        {row.originalName}
                      </td>
