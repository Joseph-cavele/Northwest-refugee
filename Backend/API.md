# NWHR Backend — how it works

The API for **North West House of Refuge**, a nonprofit in Rustenburg serving refugees,
asylum seekers and migrants. This document explains the request flow, the rules that hold
across every module, and what each router does.
...
Every request goes through the same pipeline, and the order matters:

```
requestId → helmet → cors → compression
          → [webhooks: rawBody]        ← must be ABOVE express.json()
          → express.json → cookieParser → sanitizeRequest
          → pino-http → rateLimit
...
**`rawBody` above `express.json()`** is not stylistic. PayFast signs the raw request
bytes; once JSON has parsed and re-serialised the body the signature can no longer be
reproduced. `rawBody` throws loudly if it is ever mounted below the parser.

**`sanitizeRequest`** strips `$`-prefixed and dotted keys from `req.body` and `req.params`.
`req.query` is not scrubbed — on Express 5 it is a getter that reparses on each access, so
deletions never persist. Query safety comes from each route's zod schema instead.

**`errorHandler` is the only place errors are formatted.** Nothing else calls
`res.status().json()`.
...
```
route → middleware (authenticate, authorize, validate) → controller → service → model
```

- **Controllers never import models.** They call services and shape the HTTP response.
...
- **Cross-module access is service → service.** `document.service` calls
  ...
  because that function is where row-level scoping lives.

Each module is five files: `<name>.model.js`, `.schema.js`, `.service.js`,
`.controller.js`, `.routes.js`. Some are thinner where the tree specifies it.
...
`config/permissions.js` is the single source of truth. Routes say
`authorize('transaction:approve')`. A typo'd permission string throws at wire-up rather
than silently denying everyone — which would look like a broken login.
...
`users` — staff administration beyond the invitation flow. The five files under
`modules/users/` are empty scaffolding.
...
npm run seed    # ED account + five pillar programmes; idempotent
npm test        # vitest + supertest, needs a mongod on 27017
npm run lint
```

`config/env.js` validates every variable at boot and exits with a readable list. Empty
values in `.env` are treated as absent. `ENCRYPTION_KEY` is optional at boot but
`utils/encrypt.js` throws on first use — a missing key must stop a write, never let a
permit number through in the clear.

> **Tests wipe their database.** `tests/setup.js` points at
> `mongodb://127.0.0.1:27017/nwhr-test` and `resetDatabase()` empties every collection.
> Set `TEST_MONGO_URI` if your local mongod holds development data.
